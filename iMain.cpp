...
void readscore()
{
	...
	for (int i = 0; i < 5; i++) {
		...
		iSetColor(0, 255, 255);
		iText(800, 750 - 50 * i, showname[i],
		      GLUT_BITMAP_TIMES_ROMAN_24);
		iText(1000, 750 - 50 * i, showscore[i],
		      GLUT_BITMAP_TIMES_ROMAN_24);
	}
...
void collision()
{
	if (((px + 30 >= e1x && px <= e1x + 30) && (py + 60 >= e1y && py <= e1y + 60)) ||
	    ((px + 20 >= e2x && px <= e2x + 20) && (py + 60 >= e2y && py <= e2y + 60)) ||
	    ((px + 20 >= e3x && px <= e3x + 20) && (py + 60 >= e3y && py <= e3y + 60)) ||
	    ((px + 20 >= e4x && px <= e4x + 20) && (py + 40 >= e4y && py <= e4y + 40))) {
		if (heart == 0) {
			gameoverpage = 1;
			startpage = 0;
...
void iMouse(int button, int state, int mx, int my)
{
	if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) {
		printf("x = %d, y = %d\n", mx, my);
		if (homepage == 1 && (mx >= 780 && mx <= 1070) &&
		    (my >= 400 && my <= 510))
			startbuttonclick();
		else if (homepage == 1 && (mx >= 780 && mx <= 1070) &&
			 (my >= 180 && my <= 305))
			scorebuttonclick();
		else if ((startpage == 1 || scorepage == 1 || easypage == 1 ||
			  mediumpage == 1 || hardpage == 1 || creditpage == 1)
			 && (mx >= 20 && mx <= 170) && (my >= 965 && my <= 1040))
			backbuttonclick();
		else if (gameoverpage == 1 && (mx >= 860 && mx <= 1020) &&
			 (my >= 270 && my <= 340))
			restartbuttonclick();
		else if (startpage == 1 && (mx >= 800 && mx <= 1175) &&
			   (my >= 700 && my <= 845))
			easybuttonclick();
		else if (startpage == 1 && (mx >= 800 && mx <= 1175) &&
			 (my >= 500 && my <= 645))
		...
		if (homepage == 1 && (mx >= 1760 && mx <= 1840) &&
			...
			if (musicon) {
				...
				musicon = true;
				PlaySound(TEXT("music\\music1.wav"), NULL,
					  SND_LOOP | SND_ASYNC);
			}
		}
...
/*
	function iSpecialKeyboard() is called whenver user hits special
	keys like- function keys, home, end, pg up, pg down,
	arraows etc. you have to use appropriate constants to detect them. A list is:
...
void makespace()
{
	if ((e1x - e2x) < 300 && (e1x - e2x) > 50 ||
	    (e1x - e3x) < 300 && (e1x - e3x) > 50 ||
	    (e1x - e4x) < 150 && (e1x - e4x) > 0)
		e1x = e1x + 300;

	if ((e2x - e3x) < 300 && (e2x - e3x) > 50 ||
	    (e2x - e1x) < 300 && (e2x - e1x) > 50 ||
	    (e2x - e4x) < 150 && (e2x - e4x) > 0)
		e2x = e2x + 300;

	if ((e3x - e2x) < 300 && (e3x - e2x) > 50 ||
	    (e3x - e1x) < 300 && (e3x - e1x) > 50 ||
	    (e3x - e4x) < 150 && (e3x - e4x) > 00)
		e3x = e3x + 300;

	if ((e4x - e2x) < 150 && (e4x - e2x) > 0 ||
	    (e4x - e1x) < 150 && (e4x - e1x) > 0 ||
	    (e4x - e3x) < 150 && (e4x - e3x) > 0)
		e4x = e4x + 150;
}
